...
subroutine el_timedependent_2dbasic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    ...
    real (prec)  ::  uu(6),duu(6) ,q(6)           ! Strain vector contains [e11, e22, e33, 2e12, 2e13, 2e23]
    ...
    real (prec)  :: dd,kk,the,ff,ff2,dff              ! Material properties
    !
    ...
    do kint = 1, n_points
        ...
        uu = matmul(B,dof_total)
        duu=matmul(B,dof_increment)

        ff=uu(2)*(uu(2)**2.d0-1.d0)
        dff=3.d0*(uu(2)+duu(2))**2.d0-1.d0
        ff2=(uu(2)+duu(2))*((uu(2)+duu(2))**2-1.d0)
        D=0.d0
        d(1,1)=1.d0
        d(1,2)=-dff
        d(2,2)=1.d0/dtime
        ...
        q(3)=-kk*(uu(5)+duu(5))
        q(4)=-kk*(uu(6)+duu(6))
        q(5)=dd*(uu(3)+the*duu(3))
        q(6)=dd*(uu(4)+the*duu(4))
    ...
    return
end subroutine el_timedependent_2dbasic


!==========================SUBROUTINE el_linelast_3dbasic_dynamic ==============================
subroutine el_timedependent_2dbasic_dynamic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties,element_coords, length_coord_array, &                               ! Input variables
    dof_increment, dof_total, length_dof_array,  &                                                       ! Input variables
    n_state_variables, initial_state_variables, &                                                        ! Input variables
    ...
    return
end subroutine el_timedependent_2dbasic_dynamic


!==========================SUBROUTINE fieldvars_linelast_2dbasic ==============================
subroutine fieldvars_timedependent_2dbasic(lmn, element_identifier, n_nodes, node_property_list, &           ! Input variables
    n_properties, element_properties,element_coords,length_coord_array, &                                ! Input variables
    dof_increment, dof_total, length_dof_array,  &                                                      ! Input variables
    ...
    do kint = 1, n_points
        ...
        smises = dsqrt(( (stress(1)-stress(2))**2+(stress(2)-S3)**2+(S3-stress(1))**2+6.d0*stress(3)**2)/2.d0)
    ...
    return
end subroutine fieldvars_timedependent_2dbasic
